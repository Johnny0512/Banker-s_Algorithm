...
public class Bankers_Algorithm_Solve_Alter {
    ...
    static void bank_init() throws Exception {
    ...
    static int safe_new() {
        ...
        for(int i = 0; i < p.length; i++) {
            ...
            if (!check(work,p[i].getNeed())) {
                //finding the process with the largest amount of allocated resources
                if (max==null||check(p[i].getAllocated(),max.getAllocated())){
                    max=p[i];
                }
                //Switching elements
                switch_count+=1;
                ...
                if(switch_count == p.length-i-2) {
                    ...
                    /*
                    Solving deadlock - method 2
                     */
                    stat = 1;
                    work = ArrayUtilities.arrayPlus(work,max.getAllocated());
                    max.deallocate(max.getAllocated());
                    release_count++;
                    switch_count = 0;
        ...
        System.out.println("New: safe");
        System.out.println("New time: "+(System.nanoTime()-before));
        return stat;
    }
    ...
    static int safe_origin() {
        ...
        //ArrayList<MyProcess> safe_seq = new ArrayList<>();
        while (flag) {
            flag = false;
